...
import { EntityArrayResponseType, PromotionService } from '../service/promotion.service';
...
export class PromotionComponent implements OnInit {
  ...
  trackId = (item: IPromotion): number => this.promotionService.getPromotionIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.promotions.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IPromotion[] | null): IPromotion[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.promotionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
