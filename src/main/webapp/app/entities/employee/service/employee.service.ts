...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'hiredAt'> & {
  hiredAt?: string | null;
};
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      hiredAt: employee.hiredAt?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      hiredAt: restEmployee.hiredAt ? dayjs(restEmployee.hiredAt) : undefined,
    };
  }
