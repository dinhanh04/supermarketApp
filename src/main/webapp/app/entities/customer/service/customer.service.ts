...
type RestOf<T extends ICustomer | NewCustomer> = Omit<T, 'dob' | 'createdAt' | 'updatedAt'> & {
  dob?: string | null;
...
export class CustomerService {
  ...
  protected convertDateFromClient<T extends ICustomer | NewCustomer | PartialUpdateCustomer>(customer: T): RestOf<T> {
    return {
      ...customer,
      dob: customer.dob?.format(DATE_FORMAT) ?? null,
      createdAt: customer.createdAt?.toJSON() ?? null,
      updatedAt: customer.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCustomer: RestCustomer): ICustomer {
    return {
      ...restCustomer,
      dob: restCustomer.dob ? dayjs(restCustomer.dob) : undefined,
      createdAt: restCustomer.createdAt ? dayjs(restCustomer.createdAt) : undefined,
      updatedAt: restCustomer.updatedAt ? dayjs(restCustomer.updatedAt) : undefined,
    };
  }
