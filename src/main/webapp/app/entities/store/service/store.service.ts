...
type RestOf<T extends IStore | NewStore> = Omit<T, 'createdAt' | 'updatedAt'> & {
...
export class StoreService {
  ...
  protected convertDateFromClient<T extends IStore | NewStore | PartialUpdateStore>(store: T): RestOf<T> {
    return {
      ...store,
      createdAt: store.createdAt?.toJSON() ?? null,
      updatedAt: store.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restStore: RestStore): IStore {
    return {
      ...restStore,
      createdAt: restStore.createdAt ? dayjs(restStore.createdAt) : undefined,
      updatedAt: restStore.updatedAt ? dayjs(restStore.updatedAt) : undefined,
    };
  }
