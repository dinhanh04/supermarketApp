...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    @Query(
        value = "select review from Review review left join fetch review.product left join fetch review.customer",
        countQuery = "select count(review) from Review review"
    )
    Page<Review> findAllWithToOneRelationships(Pageable pageable);

    @Query("select review from Review review left join fetch review.product left join fetch review.customer")
